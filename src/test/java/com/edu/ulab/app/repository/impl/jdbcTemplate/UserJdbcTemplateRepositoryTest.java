...
/**
 * Тесты репозитория {@link UserJdbcTemplateRepository}.
 */
@SystemJpaTest
class UserJdbcTemplateRepositoryTest {
    ...
    void insertPerson_thenAssertDmlCount() {
        ...
        assertEquals("reader1", result.getTitle());
        assertSelectCount(1);
        assertInsertCount(1);
        assertUpdateCount(0);
        assertDeleteCount(0);
    ...
    void deleteUserById_thenAssertDmlCount() {
        ...
        //Then
        assertThatThrownBy(() -> userRepository.findById(USER_ID))
                .isInstanceOf(NotFoundException.class)
                .hasMessage(String.format("User with id %s is not found.", USER_ID));
        assertSelectCount(4);
        assertInsertCount(0);
        assertUpdateCount(0);
        assertDeleteCount(3);
